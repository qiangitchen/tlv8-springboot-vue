...
public class GetServicesNetController {
	...
	public String execute() throws Exception {
		...
		for (NetworkIF net : networkIFs) {
			...
			if (hasData) {
				...
				strbf.append("<tr><td>" + name + "接收到的总字节数:</td><td>" + FormatUtil.formatBytes(net.getBytesRecv()) + "</td></tr>");// 接收到的总字节数
				strbf.append("<tr><td>" + name + "发送的总字节数:</td><td>" + FormatUtil.formatBytes(net.getBytesSent()) + "</td></tr>");// 发送的总字节数
				...
				//strbf.append("<tr><td>" + name + "发送时丢弃的包数:</td><td>" + net.getOutErrors() + "</td></tr>");// 发送时丢弃的包数
			}
			strbf.append("</table>");
		}
	...
	public static void main(String[] args) {
		try {
			GetServicesNetController test = new GetServicesNetController();
