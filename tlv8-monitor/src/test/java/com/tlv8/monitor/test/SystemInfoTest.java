...
//        double[] load = processor.getSystemLoadAverage(0);//.getProcessorCpuLoadBetweenTicks();
//        for (double avg : load) {
...
//        System.out.println(procCpu.toString());
    ...
    private static void printPowerSources(List<PowerSource> powerSources) {
        ...
        if (powerSources.size() == 0) {
            ...
            double timeRemaining = powerSources.get(0).getTimeRemainingEstimated();//.getTimeRemaining();
            if (timeRemaining < -1d) {
                sb.append("Charging");
