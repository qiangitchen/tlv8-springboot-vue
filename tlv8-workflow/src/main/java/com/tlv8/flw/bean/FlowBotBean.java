...
public class FlowBotBean {
	...
	public FlowBotBean(String flowID) throws Exception {
		...
		if (DBUtils.IsOracleDB("system")) {
			sql = "select SID,SNAME,SEPERSONNAME,SEDEPTNAME,SEOGNNAME,SSTATUSNAME,"
					...
					+ "where SSTATUSID != 'tesExecuting' and SACTIVITY !=' ' and SFLOWID='"
					+ flowID + "' order by SCREATETIME";
		} else {
			sql = "select SID,SNAME,SEPERSONNAME,SEDEPTNAME,SEOGNNAME,SSTATUSNAME,"
					+ "SCREATETIME,"
					+ "SEXECUTETIME,"
		...
		if (rslist.size() > 0) {
			...
			for (int i = 0; i < rslist.size(); i++) {
				...
				botbean.setExcutordpt(((String) m.get("SEDEPTNAME") == null
						|| "null".equals((String) m.get("SEDEPTNAME")) || ""
						.equals((String) m.get("SEDEPTNAME"))) ? (String) m
						.get("SEOGNNAME")
						: (String) m.get("SEDEPTNAME") == null ? ""
								: (String) m.get("SEDEPTNAME"));
				...
				JSONObject obj = new JSONObject(botbean);
				botdata.add(obj.toString());
			}
		} else {
	...
	public void setExcutordpt(String excutordpt) {
		this.excutordpt = (excutordpt == null || "null".equals(excutordpt)) ? ""
				: excutordpt;
	}
	...
	public String toJsonStr() throws Exception {
		String result = "";
		JSONArray json = new JSONArray(botdata);
