...
public class FlowProcess {
	...
	public List<FlowActivity> getProcessActivitys() {
		...
		try {
			JSONObject jsonObj = new JSONObject(processActy);
			sprocessacty = jsonObj.getString("nodes");
			JSONArray ActivityArray = new JSONArray(sprocessacty);
			for (int i = 0; i < ActivityArray.length(); i++) {
				JSONObject Acjson = (JSONObject) ActivityArray.get(i);
				if (!"condition".equals(Acjson.getString("type"))
						&& !"start".equals(Acjson.getString("type"))
						&& !"end".equals(Acjson.getString("type"))) {
					String actID = Acjson.getString("id");
					FlowActivity act = new FlowActivity(actMap, processID,
							actID);
					ActList.add(act);
				}
	...
	public static void main(String[] args) {
		new FlowProcess("C551A33924A00001953AE48270042B90")
				.getProcessActivitys();
	}
