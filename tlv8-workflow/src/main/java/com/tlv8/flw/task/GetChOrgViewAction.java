...
import com.tlv8.core.ActionSupport;
...
public class GetChOrgViewAction extends ActionSupport {
	...
	public String exeCreateTreeAction() throws SQLException, NamingException {
		...
		try {
			...
			rootFilter = "EXISTS(select SMANAGEORGFID from SA_OPMANAGEMENT m where SPERSONID = '"
					...
					+ "' and m.SMANAGEORGID like sa_oporg_view.SID||'%' "
					+ " and SMANAGETYPEID='systemManagement')";

			if ((!"".equals(columns)) && (columns != null))
				sql = "select " + columns + "," + id + "," + name + ","
						+ parent + " from " + table;
			else
			...
			for (int i = 0; i < rslist.size(); i++) {
				...
				while (ti.hasNext()) {
					...
					item.put("isParent",
							isParent((String) rs.get("SORGKINDID")));
	...
	public static void main(String[] args) {
		try {
			JSONObject obj = new JSONObject("{'id':'SID','name':'SNAME'}");
			obj.put("hh", "mmmm");
			Sys.printMsg(obj.get("id"));
			Sys.printMsg(obj.get("name"));
			Sys.printMsg(obj.toString());
		} catch (JSONException e) {
			e.printStackTrace();
		}
	}
