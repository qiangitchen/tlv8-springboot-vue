package com.tlv8.doc.lucene.art;
...
import com.tlv8.doc.core.FileReader;
...
public class IndexWrite {
	 ...
	 * @Author:CQ
	 */
	public static void write(FileAttribute fatr) throws Exception {
		/* 指明要索引文件 */
		...
		IndexWriterConfig iwc = new IndexWriterConfig(Version.LUCENE_36,
				lucenAnalyzer);
		iwc.setOpenMode(OpenMode.CREATE_OR_APPEND);// 创建新的索引文件create
													// 表示创建或追加到已有索引库
		...
		if (file.isFile()) {
			...
			Field fileSize = new Field("fileSize", fatr.getFileSize() + "",
					Field.Store.YES, Field.Index.NO);
			Field version = new Field("version", String.valueOf(fatr
					.getVersion()), Field.Store.YES, Field.Index.NO);
			Field addTime = new Field("addTime", new SimpleDateFormat(
					"yyyy-MM-dd HH:mm:ss").format(fatr.getAddTime()),
					Field.Store.YES, Field.Index.NO);
			Field FieldBody = new Field("body",
					fatr.getFileName() + " " + temp, Field.Store.YES,
					Field.Index.ANALYZED,
					Field.TermVector.WITH_POSITIONS_OFFSETS);
			NumericField modifiField = new NumericField("modified");// 所以key为modified
			modifiField.setLongValue(file.lastModified());
			document.add(fileID);
