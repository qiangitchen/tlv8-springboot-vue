    ...
    public Object savePermissions(@RequestBody Map<String, String> param) {
        try {
            ...
            for (String menuid : menuArray) {
                ...
                SaOpPermission permission = saOpPermissionService.selectByRoleMenu(roleid, menuid);
                boolean isnew = false;
                if (permission == null) {
                    isnew = true;
                    permission = new SaOpPermission();
                ...
                if (isnew) {
                    ...
                    saOpPermissionService.updateData(permission);
                }
            }
            for (String menuid : remidArray) {
                SaOpPermission permission = saOpPermissionService.selectByRoleMenu(roleid, menuid);
                if (permission != null) {
                    saOpPermissionService.deleteData(permission);
                }
            }
