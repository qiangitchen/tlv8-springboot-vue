...
if (!$.jpolite.Login) $.jpolite.Login = {
	...
	initCaptchaImage: function(imageID, filename){
		var capimgurl = "/system/common/captchaimage";
		function loadCapImage(){
			return $.ajax({
				type : "get",
				async : true,
				url : capimgurl+"?t="+new Date().getTime(),
				dataType : "json",
				success : function(rdata) {
					$.jpolite.Login.uuid = rdata.uuid;
					$("#" + imageID).attr("src", "data:image/jpg;base64,"+rdata.img);
				}
			});
		}
		loadCapImage();
	...
	initRememberCheckbox: function(checkboxID){
		this.items.rememberCheckbox = $("#" + checkboxID).change(function(event){
			$.cookie("justep_remember", $.jpolite.Login.items.rememberCheckbox.get(0).checked, {expires:7,path:'/'});
		});
	},
	initPasswordCheckbox: function(checkboxID){
		this.items.passwordCheckbox = $("#" + checkboxID).change(function(event){
			$.cookie("justep_password", $.jpolite.Login.items.passwordCheckbox.get(0).checked, {expires:7,path:'/'});
		});
	},
	...
	initFromCookie: function(forced){
		var remember = forced || $.cookie("justep_remember")=="true";
		...
		if (remember) {
			$.jpolite.Login.items.usernameInput.val(CryptoJS.AESDecrypt($.cookie("jpolite_username")) || "");
			$.jpolite.Login.items.languageInput.val($.cookie("jpolite_language") || "");
		}
		...
		if(password){
			$.jpolite.Login.items.passwordInput.val(CryptoJS.AESDecrypt($.cookie("jpolite_password")) || "");
		}
