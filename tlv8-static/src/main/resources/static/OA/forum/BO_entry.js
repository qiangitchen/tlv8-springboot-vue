...
function lookEntry(){
	...
	tlv8.portal.openWindow('帖子详情','/OA/forum/lookentry.html?rowid=' + rowid+"&name="+name+"&time="+time);
}
//修改内容
function editOrgData(event){
	tlv8.portal.openWindow("修改","/OA/forum/BO_entryPublish.html?gridrowid="+ event.CurrentRowId,null); 
}
//快速回复
function velocity(){
	var rowid=currentgrid.getCurrentRowId();
	tlv8.portal.openWindow("快速回复","/OA/forum/velocityentry.html?rowid="+rowid+"&title="+name,null);  
}
function openview(){
	tlv8.portal.openWindow("发帖","/OA/forum/BO_entryPublish.html?rowid="+ gridrowid+ "&tabID="+tlv8.portal.currentTabId(),null);
}
function callBackFn(){
