...
function init_grid_flow() {
	...
	var labelid = "No,sName,sflowName,sCreateTime,SEPERSONName,sAStartTime,sAFinishTime,sData1,sStatusID,sStatusName,sKindID,SCPERSONID,SCPOSID,SCDEPTID,SCOGNID,SCFID,sEURL,sActivity,SFLOWID";
	var labels = "No.,流程名称,文件名称,创建时间,创建人,sAStartTime,sAFinishTime,单据ID,状态,状态名称,sKindID,sCreatorPersonID,sCreatorPosID,sCreatorDeptID,sCreatorOgnID,sCreatorFID,sEURL,sActivity,流程ID";
	var datatype = "ro,string,ro,datetime,ro,datetime,datetime,string,string,string,ro,ro,ro,ro,ro,ro,ro,ro,ro";
	var labelwidth = "40,80,600,130,80,0,0,0,0,80,0,0,0,0,0,0,0,0,0";
	var dataAction = {
		"queryAction": "getGridAction",
		"savAction": "saveAction",
		"deleteAction": "deleteAction"
	};
	var maingrid = new tlv8.createGrid(det, labelid, labels, labelwidth, dataAction, "100%", "100%", data, 20, "", "", "", datatype, "false", "true");
	...
	myQuickItem += "<select onchange='selectDateFilter(this)' id='Time_select' lay-filter='Time_select1'>" + "<option value='all'>全部</option>" + "<option value='today'>今天</option>" + "<option value='yestoday'>昨天</option>" + "<option value='theWeek'>本周</option>" + "<option value='beforeWeek'>上周</option>" + "<option value='theMonth'>本月</option>" + "<option value='beforeMonth'>上月</option>" + "<option value='theYear'>今年</option>" + "<option value='beforeYear'>上年</option>" + "<option value='myConfirm'>自定义...</option></select></td>"; // 嵌入自定义工具条内容
	myQuickItem += "<td style='width:120px;padding-right:5px;'>" + "<select onchange='selectStatusFilter(this)' id='status_select' lay-filter='status_select1'>" + "<option value='tesReady'>正在处理</option>" + "<option value='tesFinished'>已完成</option>" + "<option value='all'>全部</option>" + "<option value='tesReturned'>已回退</option>" + "<option value='tesCanceled'>已取消</option>" + "<option value='tesPause'>已暂停</option>" + "<option value='tesAborted'>已终止</option></select></td>";

	var currentPsnId = tlv8.Context.getCurrentPersonID();
	myQuickItem += "<td style='width:180px;padding-right:5px;'>" + "<select onchange='selectOrgFilter(this)' id='Org_select' lay-filter='Org_select1' " + "value='本人' filterval=\"(SCPERSONID ='" + currentPsnId + "' or SEPERSONID = '" + currentPsnId + "' or EXISTS(select 1 from SA_Task ts where ts.sflowid = SA_TASK_MONITOR.sflowid" + " and (ts.SCPERSONID = '" + currentPsnId + "' or SEPERSONID = '" + currentPsnId + "')) and SEPERSONID is not null)\">" + "<option value='本人'>本人</option>" + "<option value='select'>选择...</option></select></td>";
	myQuickItem += "</td></tr></table>";
	myQuickItem += "</form>";
	document.getElementById(det.id + "-grid-item").innerHTML = myQuickItem;
	layui.form.render("select");

	layui.form.on('select(Time_select1)',
	function(data) {
		selectDateFilter(data.elem);
	});
	layui.form.on('select(status_select1)',
	function(data) {
		selectStatusFilter(data.elem);
	});
	layui.form.on('select(Org_select1)',
	function(data) {
	...
	grid_flow.setStaticFilter("(SCPERSONID ='" + currentPsnId + "' or SEPERSONID = '" + currentPsnId + "' or EXISTS(select 1 from SA_Task ts where ts.sflowid = SA_TASK_MONITOR.sflowid" + " and (ts.SCPERSONID = '" + currentPsnId + "' or SEPERSONID = '" + currentPsnId + "')) and SEPERSONID is not null) and (SSTATUSID = 'tesExecuting' or SSTATUSID = 'tesReady')");
...
function selectDateFilter(obj) {
	...
	if (obj.value == "all") {
		...
		tlv8.portal.dailog.openDailog("自定义...", "/system/task/dialog/MyDateConfig.html", 300, 100, myConfigDatebackFn, {
			refreshItem: false,
			enginItem: true,
			CanclItem: true
		});
	...
	if (begin && end) {
		dateFilter = "sCreateTime >= to_date('" + begin.format('yyyy-MM-dd') + "','yyyy-MM-dd') and sCreateTime <= to_date('" + end.format('yyyy-MM-dd') + "','yyyy-MM-dd')";
	}
	var temp_dateFilter = dateFilter;
	if (state_filter_val && state_filter_val != "" && dateFilter && dateFilter != "") {
...
function myConfigDatebackFn(redata) {
	if (redata) {
		dateFilter = "sCreateTime >= to_date('" + redata.begin.format('yyyy-MM-dd') + "','yyyy-MM-dd') and sCreateTime <= to_date('" + redata.end.format('yyyy-MM-dd') + "','yyyy-MM-dd')";
...
function selectStatusFilter(obj) {
	...
	if (dateFilter && dateFilter != "" && state_filter_val && state_filter_val != "") {
...
function selectOrgFilter(obj) {
	if (obj.value == "select") {
		tlv8.portal.dailog.openDailog("机构选择", "/comon/SelectDialogPsn/SelectChOrgView.html", 500, 350,
		function(rData) {
			org_Select_filter = "(";
			var sOrg_Value = "";
			for (var i in rData) {
				...
				org_Select_filter += " SCFID like '" + rData[i].SFID + "%' or SEFID like '" + rData[i].SFID + "%'";
				sOrg_Value += rData[i].name;
			}
			org_Select_filter += ")";
			$("#Org_select").attr("filterval", org_Select_filter);
			$("#Org_select").html("<option value='本人'>本人</option>" + "<option value='select'>选择...</option>" + "<option value='" + sOrg_Value + "'>" + sOrg_Value + "</option>");
			...
			if (state_filter_val && state_filter_val != "") {
				tem_org_Select_filter += state_filter_val + " and " + tem_org_Select_filter;
			}
		...
		var currentPsnId = tlv8.Context.getCurrentPersonID();
		org_Select_filter = "(SCPERSONID ='" + currentPsnId + "' or SEPERSONID = '" + currentPsnId + "' or EXISTS(select 1 from SA_Task ts where ts.sflowid = SA_TASK_MONITOR.sflowid" + " and (ts.SCPERSONID = '" + currentPsnId + "' or SEPERSONID = '" + currentPsnId + "')) and SEPERSONID is not null)";
		$("#Org_select").attr("filterval", org_Select_filter);
		var tem_org_Select_filter = "";
		if (dateFilter && dateFilter != "") {
			tem_org_Select_filter += dateFilter + " and " + org_Select_filter;
...
function init_grid_taskWindow(flowid) {
	...
	var labelid = "No,sName,SEPERSONName,SCPERSONName,sAStartTime,sAFinishTime,sData1,sStatusID,sStatusName" + ",sKindID,SCPERSONID,SCPOSID,SCDEPTID,SCOGNID,SCFID,sEURL,sActivity,SEPERSONID";
	var labels = "No.,名称,执行人,提交人,开始时间,结束时间,单据ID,状态,状态名称" + ",sKindID,sCreatorPersonID,sCreatorPosID,sCreatorDeptID,sCreatorOgnID,sCreatorFID,sEURL,sActivity,SEPERSONID";
	var datatype = "ro,string,string,string,datetime,datetime,string,string,string,ro" + ",ro,ro,ro,ro,ro,ro,ro,ro";
	...
	var maingrid = new tlv8.createGrid(det, labelid, labels, labelwidth, dataAction, "100%", "100%", data, 20, filter, "", "", datatype, "false", "true");
	grid_task = maingrid.grid;
	grid_task.seteditModel(false);
	grid_task.settoolbar(false, false, true, false);
	do_task_Item = grid_task.insertSelfBar("处理任务", "90px", "do_task()", "../../../comon/image/toolbar/flw/do_task.gif");
	...
	changeExcutor_Item = grid_task.insertSelfBar("修改执行人", "100px", "changeExcutor()");
...
function SpecFlowout() {
	...
	tlv8.XMLHttpRequest("GetSpecFlowoutInfoAction", param, "post", true,
	function(r) {
		if (r.data.flag == "true") {
			try {
				var reActData = eval("(" + r.data.data + ")");
				var activityListStr = reActData.activityListStr;
				if (activityListStr == "[]") return false;
				...
				tlv8.portal.dailog.openDailog('流程信息', exe_selct_url, 800, 600, flowEngion, null, null, activityListStr);
			} catch(e) {
...
function puse_ItemFlow() {
	grid_task.setValueByName("sStatusID", grid_task.CurrentRowId, "tesCanceled");
...
function changeExcutor() {
	...
	tlv8.portal.dailog.openDailog("选择执行人", "/comon/SelectDialogPsn/singleSelectPsn.html", 500, 350,
	function(re) {
		var param = new tlv8.RequestParam();
		param.set("taskID", taskID);
		param.set("epersonids", re.rowid);
		tlv8.XMLHttpRequest("ChangeFlowExcutorAction", param, "POST", true,
		function(r) {
			if (r.data.flag == "false") {
				alert(r.data.message);
			} else {
...
function tab_flowData() {
	...
	if (url && url != "") {
		url += "?flowID=" + flowID + "&taskID=" + taskID + "&sData1=" + sData1 + "&task=" + taskID + "&activity-pattern=detail";
		if (url.indexOf(cpath + "/") < 0) {
			url = cpath + url + "&activity-pattern=detail&bsessionid=" + parent.$.jpolite.clientInfo.businessid + "&language=zh_CN&personid=" + SEPERSONID + "&$portlet-state=portler:p";
		}
	}
	if (!sEURL || sEURL == "") {
		frame_flowData.src = "";
