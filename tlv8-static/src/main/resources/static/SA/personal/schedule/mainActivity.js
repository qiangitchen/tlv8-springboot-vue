...
function cycleAffairs() {
	var gridData = new tlv8.Data();
	gridData.setTable("SA_PSNSCHEDULE");
	gridData.setDbkey("system");
	gridData.setOrderby("SSTARTDATE");
	var where = "SAFFAIRSTYPE=3";// 过滤条件
	...
	var dataAction = {
		"queryAction" : "getGridAction",// 使用SQL语句"getGridActionBySQL",
		"savAction" : "",
		"deleteAction" : ""
	};
	var maingrid = new tlv8.createGrid(d, labelid, labels, labelwidth,
			dataAction, "100%", "100%", gridData, 20, where, "", "", datatype,
			"true", "true", "");// 多选、序号
	// 设置按钮显示{新增、保存、刷新、删除}
	maingrid.grid.settoolbar(false, false, true, true);
	// 设置是否可编辑
	maingrid.grid.seteditModel(false);
	cycleAffairsgrid = maingrid.grid;
	cycleAffairsgrid.setExcelimpBar(true);// 导入
	cycleAffairsgrid.setExcelexpBar(true);// 导出
}
...
function myTaskLoad() {
	...
	if (myTaskLoadData != null && myTaskLoadData != undefined) {
		for (var i = 0; i < myTaskLoadData.length; i++) {
			try {
				var sid = myTaskLoadData[i].SID, caption = myTaskLoadData[i].SCAPTION, startdate = myTaskLoadData[i].SSTARTDATE
						.substring(0, 19), enddate = myTaskLoadData[i].SENDDATE
						.substring(0, 19), priority = myTaskLoadData[i].SPRIORITY, content = myTaskLoadData[i].SCONTENT, status = myTaskLoadData[i].SSTATUS, affairstype = myTaskLoadData[i].SAFFAIRSTYPE, completerate = myTaskLoadData[i].SCOMPLETERATE, startdate_axis = myTaskLoadData[i].SSTARTDATE_AXIS, senddate_axis = myTaskLoadData[i].SSENDDATE_AXIS, swhouser = myTaskLoadData[i].SWHOUSER;
