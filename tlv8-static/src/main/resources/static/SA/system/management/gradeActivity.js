...
function initManagaTypeSelectData() {
	var mgResult = tlv8.XMLHttpRequest("system/manage/loadManagementType", null, "post", false);
	var managaTypeSelectOptions = "";
	for ( var i in mgResult.data) {
		managaTypeSelectOptions += "<option value='" + mgResult.data[i].id
				+ "'>" + mgResult.data[i].text + "</option>";
	}
...
function managaTypePerMision() {
	tlv8.portal.dailog.openDailog("管理类型",
			"/SA/system/management/dialog/managetype-permision.html", 560, 350,
			function(r) {
				initManagaTypeSelectData();
			});
}
...
function new_permition(orgID) {
	tlv8.portal.dailog.openDailog("选择管理范围 可分配至人员",
			"/SA/system/management/dialog/grade-select-org-dialog.html", 600, 350,
