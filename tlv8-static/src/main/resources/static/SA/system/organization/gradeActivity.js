...
var param = {
	cell : {
		...
		rootFilter : " SID in (select SMANAGEORGID from SA_OPMANAGEMENT where SORGFID = '"
...
function getData() {
	...
	setTimeout(function() {
		try {
			var pnode = MainJtree.getNodeByTId(tlv8.Context.getCurrentOgnID());
			MainJtree.selectNode(pnode, true);
		} catch (e) {
		}
	}, 1000);
}
...
function newognData() {
	...
	tlv8.portal.dailog.openDailog('新建组织',
			"/SA/system/organization/dialog/organ-org-createorg.html?rowid="
...
function newPsmData() {
	...
	tlv8.portal.dailog.openDailog('新建人员',
			"/SA/system/organization/dialog/organ-psm-createpsm.html?rowid="
					+ currenttreeID + "&name=" + J_u_encode(currenttreeName)
					+ "&scode=" + scode + "&sfname=" + J_u_encode(sfname)
...
function assignPsmData() {
	...
	tlv8.portal.dailog.openDailog('分配人员',
			"/SA/system/dialogs/selectMultiPsm/gradeSelectChPsm.html?rowid="
...
function editOrgData(event) {
	...
	if (SORGKINDID == "psm") {
		tlv8.portal.dailog
				.openDailog('人员信息',
						"/SA/system/organization/dialog/organ-psm-createpsm.html?gridrowid="
		...
		tlv8.portal.dailog.openDailog('机构管理',
				"/SA/system/organization/dialog/organ-org-createorg.html?gridrowid="
...
function moveOrg() {
	...
	tlv8.portal.dailog.openDailog('移动机构',
			"/SA/system/organization/grademoveOrg.html?rowid=" + rowid, 400, 350,
			dailogcallback, null);
}
/*
...
function sortOrgAction() {
	...
	tlv8.portal.dailog.openDailog('机构排序',
			"/SA/system/organization/sortOrgs.html?rowid=" + rowid + "&name="
