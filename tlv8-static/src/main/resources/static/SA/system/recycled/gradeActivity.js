...
function getData() {
	...
	// 暂时只考虑Oracle数据库
	var staticFilter = "SVALIDSTATE=-1 and exists(select SMANAGEORGID from SA_OPMANAGEMENT m where m.SORGFID = '"
			+ tlv8.Context.getCurrentPersonFID()
			+ "' and m.SMANAGETYPEID='systemManagement' and SFID like m.SMANAGEORGFID||'%')";
	var maingrid = new tlv8.createGrid(d, labelid, labels, labelwidth,
			dataAction, "100%", "100%", data, 20, staticFilter, "", "",
			datatype, "true", "true");
	// 设置按钮显示{新增、保存、刷新、删除}
