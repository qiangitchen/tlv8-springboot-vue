 ...
 */
function openFlow() {
	var url = "dialog/process-select.html";
...
function saveFlowData(Love) {
	var sdrawlg = document.getElementById("group").innerHTML;
	var sprocessacty = Love.toJson().toString().encodeSpechars();
	var param = new tlv8.RequestParam();
	param.set("sprocessid", Love.id);
	param.set("sprocessname", Love.name);
	param.set("sdrawlg", CryptoJS.AESEncrypt(J_u_encode(sdrawlg)));
	param.set("sprocessacty", CryptoJS.AESEncrypt(J_u_encode(sprocessacty)));
...
function selectexePage() {
	var url = "dialog/func-tree-select.html";
...
function selectexeLabel() {
	...
	var url = "dialog/expression-editer.html?Olexpression="
			+ Olexpression;
...
function selectConditionExpression() {
	...
	var url = "dialog/expression-editer.html?operatType=condition&Olexpression="
			+ Olexpression;
	tlv8.portal.dailog.openDailog("转发规则-表达式编辑器", url, 1000, 700,
			function(data) {
				try {
