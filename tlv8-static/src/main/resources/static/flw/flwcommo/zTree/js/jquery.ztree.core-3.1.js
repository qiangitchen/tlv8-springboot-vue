...
(function($){
	...
	_initNode = function(setting, level, n, parentNode, isFirstNode, isLastNode, openFlag) {
		...
		n.level = level;
		n.tId = n.id; //setting.treeId + "_" + (++zId);
	...
	view = {
		addNodes: function(setting, parentNode, newNodes, isSilent) {
			...
			if (parentNode) {
				var target_switchObj = $("[id='" + parentNode.tId + consts.id.SWITCH+"']"),
				target_icoObj = $("[id=" + parentNode.tId + consts.id.ICON+"']"),
				target_ulObj = $("[id='" + parentNode.tId + consts.id.UL+"']");
		...
		appendParentULDom: function(setting, node) {
			var html = [],
			nObj = $("[id='" + node.tId+"']"),
			ulObj = $("[id='" + node.tId + consts.id.UL+"']"),
			childKey = setting.data.key.children,
		...
		asyncNode: function(setting, node, isSilent, callback) {
			...
			if (node) {
				node.isAjaxing = true;
				var icoObj = $("[id='" + node.tId + consts.id.ICON+"']");
		...
		cancelPreSelectedNode: function (setting, node) {
			...
			for (var i=0, j=list.length-1; j>=i; j--) {
				if (!node || node === list[j]) {
					$("[id='" + list[j].tId + consts.id.A+"']").removeClass(consts.node.CURSELECTED);
		...
		createNodes: function(setting, level, nodes, parentNode) {
			...
			var root = data.getRoot(setting),
			childKey = setting.data.key.children,
			openFlag = !parentNode || parentNode.open || !!$("[id='" + parentNode[childKey][0].tId+"']").get(0);
			root.createdNodes = [];
			var zTreeHtml = view.appendNodes(setting, level, nodes, parentNode, true, openFlag);
			if (!parentNode) {
				setting.treeObj.append(zTreeHtml.join(''));
			} else {
				var ulObj = $("[id='" + parentNode.tId + consts.id.UL+"']");
				if (ulObj.get(0)) {
		...
		expandCollapseNode: function(setting, node, expandFlag, animateFlag, callback) {
			...
			if (!node.open && node.isParent && ((!$("[id='" + node.tId + consts.id.UL+"']").get(0)) || (node[childKey] && node[childKey].length>0 && !$("[id='" + node[childKey][0].tId+"']").get(0)))) {
				view.appendParentULDom(setting, node);
			}
			var ulObj = $("[id='" + node.tId + consts.id.UL+"']"),
			switchObj = $("[id='" + node.tId + consts.id.SWITCH+"']"),
			icoObj = $("[id='" + node.tId + consts.id.ICON+"']");
		...
		selectNode: function(setting, node, addFlag) {
			...
			$("[id='" + node.tId + consts.id.A+"']").addClass(consts.node.CURSELECTED);
			data.addSelectedNode(setting, node);
		},
		setNodeFontCss: function(setting, treeNode) {
			var aObj = $("[id='" + treeNode.tId + consts.id.A+"']"),
		...
		setNodeLineIcos: function(setting, node) {
			if (!node) return;
			var switchObj = $("[id='" + node.tId + consts.id.SWITCH+"']"),
			ulObj = $("[id='" + node.tId + consts.id.UL+"']"),
			icoObj = $("[id='" + node.tId + consts.id.ICON+"']"),
		...
		setNodeName: function(setting, node) {
			...
			titleKey = data.getTitleKey(setting),
			nObj = $("[id='" + node.tId + consts.id.SPAN+"']");
			nObj.empty();
			...
			if (tools.apply(setting.view.showTitle, [setting.treeId, node], setting.view.showTitle) && node[titleKey]) {
				var aObj = $("[id='" + node.tId + consts.id.A+"']");
				...
				expandNode : function(node, expandFlag, sonSign, focus, callbackFlag) {
					...
					if (sonSign) {
						view.expandCollapseSonNode(this.setting, node, expandFlag, true, function() {
							if (focus !== false) {$("[id='" + node.tId + consts.id.ICON+"']").focus().blur();}
						});
					} else {
					...
					if (tools.uCanDo(this.setting)) {
						...
						if (node.parentTId) {
							view.expandCollapseParentNode(this.setting, node.getParentNode(), true, false, function() {
								$("[id='" + node.tId + consts.id.ICON+"']").focus().blur();
							});
						} else {
							$("[id='" + node.tId + consts.id.ICON+"']").focus().blur();
						}
