...
tlv8.HtmlPrint = {
	...
	docType : function() {
		if (this.settings.mode == this.modes.iframe || !this.settings.strict)
			return "";

		var standard = this.settings.strict == false ? " Trasitional" : "";
		var dtd = this.settings.strict == false ? "loose" : "strict";

		return '<!DOCTYPE HTML PUBLIC "-//W3C//DTD XHTML 1.0' + standard
	...
	printFlowChart : function(flowId, directPrint) {
		...
		html = html.replace('流程轨迹', '');
		html = html.replace('波特图', '');
		html = html.replace('流程轨迹', '');
		html = html.replace('波特图', '');
	...
	executePrintout : function(formId, appendHtml, closeWin, isPreview, options) {
		if (!formId) {
			tlv8.showMessage("没有指定target-id");
			return;
		}
		var modes = {
			iframe : "iframe",
	...
	transHtml : function(node) {
		...
		if (!window.event) { //chrome下fck有问题 需要进一步确认
			//fck 
			$(node)
					.find("iframe")
					.each(
