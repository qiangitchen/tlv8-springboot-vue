...
public class XlsUtils {
	...
	private static void printlntest(String value) throws UnsupportedEncodingException {
	...
	public static String test() throws Exception {
		...
		WritableSheet sheet1 = XlsUtils.createSheet(wb, "Sheet" + Integer.toString(i), i);
		...
		WritableSheet sheet2 = XlsUtils.createSheet(wb, "Sheet" + Integer.toString(i + 1), i + 1);
		...
		XlsUtils.printlntest(String.format("sheet1-cells[0,0]=%s", XlsUtils.getCellValue(rs1, 0, 0)));
		XlsUtils.printlntest(String.format("sheet1-cells[1,0]=%s", XlsUtils.getCellValue(rs1, 1, 0)));
		XlsUtils.printlntest(String.format("sheet1-cells[0,1]=%s", XlsUtils.getCellValue(rs1, 0, 1)));
		XlsUtils.printlntest(String.format("sheet1-cells[1,1]=%s", XlsUtils.getCellValue(rs1, 1, 1)));
		Sheet rs2 = XlsUtils.getSheet(rb, i + 1);
		XlsUtils.printlntest(String.format("sheet2-cells[0,0]=%s", XlsUtils.getCellValue(rs2, 0, 0)));
		XlsUtils.printlntest(String.format("sheet2-cells[1,0]=%s", XlsUtils.getCellValue(rs2, 1, 0)));
		XlsUtils.printlntest(String.format("sheet2-cells[0,1]=%s", XlsUtils.getCellValue(rs2, 0, 1)));
	...
	// 创建一个可写的Workbook
	public static WritableWorkbook createWorkbook(String fileName) throws Exception, IOException {
	...
	public static WritableSheet createSheet(WritableWorkbook workbook, String sheetName, int sheetNum) {
		WritableSheet sheet = null;
		sheet = workbook.createSheet(sheetName, sheetNum);
		return sheet;
	}

	public static void writeString(WritableSheet sheet, int col, int row, String value) throws Exception {
		jxl.write.Label label = new jxl.write.Label(col, row, value);
		sheet.addCell(label);
	}

	public static void writeNum(WritableSheet sheet, int col, int row, double value) throws Exception {
	...
	public static int colName2Index(String colName) throws Exception {
		...
		if (cname.length() < 1 || cname.length() > 2)
			throw new Exception(String.format("列名称错误, 应该是A-ZZ, 实际: %s", colName));
		if (cname.length() == 1) {
