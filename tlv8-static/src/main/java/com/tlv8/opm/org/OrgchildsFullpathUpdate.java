			...
			for (int i = 0; i < chd.size(); i++) {
				...
				try {
					...
					ps = session.getConnection().prepareStatement(
							"select t.SFID,o.SID,o.SORGKINDID,t.SFCODE,o.SCODE,t.SFNAME,o.SNAME from SA_OPOrg t,SA_OPOrg o where t.sID=? and o.SID=?");
					ps.setString(1, parent);
					ps.setString(2, orgid);
					rs = ps.executeQuery();
					if (rs.next()) {
						PreparedStatement ps1 = conn
								.prepareStatement("update SA_OPOrg set SFID=?,SFCODE=?,SFNAME=? where SID=?");
						ps1.setString(1,
								rs.getString("SFID") + "/" + rs.getString("SID") + "." + rs.getString("SORGKINDID"));
