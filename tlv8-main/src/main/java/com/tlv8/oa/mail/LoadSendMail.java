...
@Controller
@Scope("prototype")
public class LoadSendMail extends ActionSupport {
	...
	public Object execute() throws Exception {
		...
		String sql = "SELECT FID,'已发送' FQUREY,FSENDDEPT,FEMAILNAME,to_char(FCONSIGNEE) FSENDPERNAME,FSENDTIME,'this_week' AS WEEK,FCOLLECT FROM OA_EM_SENDEMAIL  WHERE FSENDPERID = '"
				+ con.getCurrentPersonID() + "' AND TRUNC(FSENDTIME) > TRUNC(SYSDATE-" + c_day + ")";
		sql += " UNION ";
		sql += "SELECT FID,'已发送' FQUREY,FSENDDEPT,FEMAILNAME,to_char(FCONSIGNEE) FSENDPERNAME,FSENDTIME,'last_week' AS WEEK,FCOLLECT FROM OA_EM_SENDEMAIL  WHERE FSENDPERID = '"
				+ con.getCurrentPersonID() + "' AND TRUNC(FSENDTIME) <= TRUNC(SYSDATE-" + c_day
				+ ")  AND TRUNC(FSENDTIME) > TRUNC(SYSDATE-" + c_day + "-7)";
		sql += " UNION ";
		sql += "SELECT FID,'已发送' FQUREY,FSENDDEPT,FEMAILNAME,to_char(FCONSIGNEE) FSENDPERNAME,FSENDTIME,'earlier' AS WEEK,FCOLLECT FROM OA_EM_SENDEMAIL  WHERE FSENDPERID = '"
				+ con.getCurrentPersonID() + "' AND TRUNC(FSENDTIME) <= TRUNC(SYSDATE-" + c_day
				+ "-7) AND ROWNUM<=20 ORDER BY FSENDTIME DESC";
		if (DBUtils.IsMySQLDB("oa")) {
			sql = "SELECT FID,'已发送' FQUREY,FSENDDEPT,FEMAILNAME,FCONSIGNEE FSENDPERNAME,FSENDTIME,'this_week' AS WEEK,FCOLLECT FROM OA_EM_SENDEMAIL  WHERE FSENDPERID = '"
					+ con.getCurrentPersonID() + "' AND FSENDTIME > date_sub(curdate(),interval -" + c_day + " day)";
			sql += " UNION ";
			sql += "SELECT FID,'已发送' FQUREY,FSENDDEPT,FEMAILNAME,FCONSIGNEE FSENDPERNAME,FSENDTIME,'last_week' AS WEEK,FCOLLECT FROM OA_EM_SENDEMAIL  WHERE FSENDPERID = '"
					+ con.getCurrentPersonID() + "' AND FSENDTIME <= date_sub(curdate(),interval -" + c_day
					+ " day)  AND FSENDTIME > date_sub(curdate(),interval -" + (c_day + 7) + " day)";
			sql += " UNION ";
			sql += "SELECT FID,'已发送' FQUREY,FSENDDEPT,FEMAILNAME,FCONSIGNEE FSENDPERNAME,FSENDTIME,'earlier' AS WEEK,FCOLLECT FROM OA_EM_SENDEMAIL  WHERE FSENDPERID = '"
					+ con.getCurrentPersonID() + "' AND FSENDTIME <= date_sub(curdate(),interval -" + (c_day + 7)
					+ " day) ORDER BY FSENDTIME DESC";
		} else if (DBUtils.IsMSSQLDB("oa")) {
			sql = "SELECT FID,'已发送' FQUREY,FSENDDEPT,FEMAILNAME,CONVERT(varchar(4000),FCONSIGNEE) FSENDPERNAME,FSENDTIME,'this_week' AS WEEK,FCOLLECT FROM OA_EM_SENDEMAIL  WHERE FSENDPERID = '"
					+ con.getCurrentPersonID() + "' AND FSENDTIME > DATEADD(DAY, -" + c_day + ",GETDATE())";
			sql += " UNION ";
			sql += "SELECT FID,'已发送' FQUREY,FSENDDEPT,FEMAILNAME,CONVERT(varchar(4000),FCONSIGNEE) FSENDPERNAME,FSENDTIME,'last_week' AS WEEK,FCOLLECT FROM OA_EM_SENDEMAIL  WHERE FSENDPERID = '"
					+ con.getCurrentPersonID() + "' AND FSENDTIME <= DATEADD(DAY,-" + c_day
					+ ", GETDATE())  AND FSENDTIME > DATEADD(DAY, -" + (c_day + 7) + ", GETDATE())";
			sql += " UNION ";
			sql += "SELECT FID,'已发送' FQUREY,FSENDDEPT,FEMAILNAME,CONVERT(varchar(4000),FCONSIGNEE) FSENDPERNAME,FSENDTIME,'earlier' AS WEEK,FCOLLECT FROM OA_EM_SENDEMAIL  WHERE FSENDPERID = '"
			...
			sql += "SELECT FID,'已发送' FQUREY,FSENDDEPT,FEMAILNAME,FCONSIGNEE as FSENDPERNAME,FSENDTIME,'last_week' AS WEEK,FCOLLECT FROM OA_EM_SENDEMAIL  WHERE FSENDPERID = '"
					+ con.getCurrentPersonID() + "' AND FSENDTIME <= NOW()::TIMESTAMP+ '-" + (c_day)
					+ " day'  AND FSENDTIME > NOW()::TIMESTAMP+ '-" + (c_day + 7) + " day'";
			sql += " UNION ";
			sql += "SELECT FID,'已发送' FQUREY,FSENDDEPT,FEMAILNAME,FCONSIGNEE as FSENDPERNAME,FSENDTIME,'earlier' AS WEEK,FCOLLECT FROM OA_EM_SENDEMAIL  WHERE FSENDPERID = '"
					+ con.getCurrentPersonID() + "' AND FSENDTIME <= NOW()::TIMESTAMP+ '-" + (c_day + 7)
					+ " day' ORDER BY FSENDTIME DESC";
		}
		try {
