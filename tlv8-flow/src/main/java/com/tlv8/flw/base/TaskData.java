...
public class TaskData {
	 ...
	 * 启动流程
	 */
	public static String startFlow(String processID, String processName, String sData1, ContextBean context)
			throws SQLException {
	...
	public static String FlowOut(String processID, String processName, String Activity, String flowID, String taskID,
		...
		if (ePersonList.size() > 0) {
			...
			JSONObject json = getExcutor(eURL, context.getCurrentPersonID());
			if (json == null) {
	...
	public static String FlowTransmit(String processID, String processName, String Activity, String flowID,
			String taskID, String sData1, List<OrgUtils> ePersonList, ContextBean context) throws SQLException {
		FlowActivity act = new FlowActivity(processID, Activity);
		processName = act.getActivityname() + ":" + processName;// 任务标题
		String eURL = act.getUrl();
		String cURL = eURL;
	...
	public static JSONObject getExcutor(String eUrl, String personid) {
		String sql = "select SCHECKPSN from SA_FLOWTRACE where SOPERATORID = '" + personid + "' and SEURL ='" + eUrl
				+ "'";
		try {
			List<Map<String, String>> li = DBUtils.execQueryforList("system", sql);
			if (li.size() > 0) {
